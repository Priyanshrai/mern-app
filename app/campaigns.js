        ...
        <ul className="list-group mx-auto" style={{ maxWidth: "800px" }}>
          {campaigns.map((campaign, index) => (
            <li key={campaign.id} className="list-group-item">
              <strong>{campaign.name}</strong> - Launched on{" "}
              {new Date().toLocaleDateString()} - Budget: ${campaign.budget} -
              Reach: {engagementData[index]?.reach} - Engagement Rate:{" "}
              {engagementData[index]?.engagementRate.toFixed(2)}%
            </li>
          ))}
        </ul>
      </div>
